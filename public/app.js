...
const els = {
  ...
  dashTotalLearners: document.getElementById("dashTotalLearners"),
  dashBoys: document.getElementById("dashBoys"),
  dashGirls: document.getElementById("dashGirls"),
  dashTeachers: document.getElementById("dashTeachers"),
  dashClasses: document.getElementById("dashClasses"),
  dashStreams: document.getElementById("dashStreams"),
  dashPassRate: document.getElementById("dashPassRate"),
  dashFailRate: document.getElementById("dashFailRate"),
  dashPromotionRate: document.getElementById("dashPromotionRate"),
  dashTopClass: document.getElementById("dashTopClass"),
  dashTopClassScore: document.getElementById("dashTopClassScore"),
  dashTopStream: document.getElementById("dashTopStream"),
  dashTopStreamScore: document.getElementById("dashTopStreamScore"),
  dashBestSubject: document.getElementById("dashBestSubject"),
  dashBestSubjectScore: document.getElementById("dashBestSubjectScore"),
  dashLowestSubject: document.getElementById("dashLowestSubject"),
  dashLowestSubjectScore: document.getElementById("dashLowestSubjectScore"),
  dashboardRecentUploads: document.getElementById("dashboardRecentUploads"),
  dashboardActivityFeed: document.getElementById("dashboardActivityFeed"),
  ...
  assessmentWorkflowStatus: document.getElementById("assessmentWorkflowStatus"),
  assessmentWorkflowDetail: document.getElementById("assessmentWorkflowDetail"),
  submitAssessmentBtn: document.getElementById("submitAssessmentBtn"),
  approveAssessmentBtn: document.getElementById("approveAssessmentBtn"),
  lockAssessmentBtn: document.getElementById("lockAssessmentBtn"),
  reopenAssessmentBtn: document.getElementById("reopenAssessmentBtn"),
  ...
  rollbackPromotionBtn: document.getElementById("rollbackPromotionBtn"),
  ...
  promotionRuleForm: document.getElementById("promotionRuleForm"),
  promotionRuleClassSelect: document.getElementById("promotionRuleClassSelect"),
  promotionMandatorySubjectsSelect: document.getElementById("promotionMandatorySubjectsSelect"),
  ...
  reportArchiveSearchInput: document.getElementById("reportArchiveSearchInput"),
  reportArchiveList: document.getElementById("reportArchiveList"),
...
function photoSrc(value) {
  if (!value) return "";
  return String(value).startsWith("storage:") ? `/api/files?ref=${encodeURIComponent(value)}` : value;
}
...
async function loadData() {
  ...
  try {
    ...
    renderAll();
    applyUrlRoute();
    if (hasRole("Super Admin", "School Admin")) await loadUsers();
  } catch (error) {
...
function showPublicPortal() {
  ...
  const match = window.location.pathname.match(/^\/verify\/(.+)$/) || window.location.hash.match(/^#verify\/(.+)$/);
  if (match) {
...
function renderSchoolMeta() {
  ...
  setValue("schoolSubscriptionPlanInput", db.school.subscriptionPlan);
  setValue("schoolSubscriptionStatusInput", db.school.subscriptionStatus);
  setValue("schoolTrialEndsInput", db.school.trialEndsAt);
  setValue("schoolSubscriptionExpiresInput", db.school.subscriptionExpiresAt);
...
function renderSelects() {
  ...
  setOptions(els.promotionRuleClassSelect, db.classes.map((item) => `<option value="${item.id}">${escapeHtml(item.name)}</option>`).join(""), db.classes[0]?.id);
  if (els.promotionMandatorySubjectsSelect) els.promotionMandatorySubjectsSelect.innerHTML = subjects;
...
function renderDashboard() {
  ...
  els.dashTotalLearners.textContent = executive.totalLearners || results.counts.students || 0;
  els.metricStudents.textContent = results.counts.activeStudents;
  els.dashBoys.textContent = executive.boys || 0;
  els.dashGirls.textContent = executive.girls || 0;
  els.dashTeachers.textContent = executive.teachers || results.counts.teachers || 0;
  els.dashClasses.textContent = executive.classes || 0;
  els.dashStreams.textContent = executive.streams || 0;
  ...
  els.dashPassRate.textContent = `${executive.passRate || 0}%`;
  els.dashFailRate.textContent = `${executive.failRate || 0}%`;
  els.dashPromotionRate.textContent = `${executive.promotionRate || 0}%`;
  els.dashTopClass.textContent = executive.topClass?.name || "-";
  els.dashTopClassScore.textContent = executive.topClass ? `${executive.topClass.average} average` : "No results yet";
  els.dashTopStream.textContent = executive.topStream?.name || "-";
  els.dashTopStreamScore.textContent = executive.topStream ? `${executive.topStream.average} average` : "No results yet";
  els.dashBestSubject.textContent = executive.topSubject?.subjectName || "-";
  els.dashBestSubjectScore.textContent = executive.topSubject ? `${executive.topSubject.average} average | ${executive.topSubject.passRate}% pass` : "No results yet";
  els.dashLowestSubject.textContent = executive.lowestSubject?.subjectName || "-";
  els.dashLowestSubjectScore.textContent = executive.lowestSubject ? `${executive.lowestSubject.average} average | ${executive.lowestSubject.passRate}% pass` : "No results yet";
  els.dashboardRecentUploads.innerHTML = (executive.recentUploads || []).map((upload) => `
    <article>
      <span class="activity-icon">UP</span>
      <div><strong>${escapeHtml(upload.subjectName || "Marks upload")}</strong><span>${escapeHtml(upload.className || "-")} | ${escapeHtml(upload.teacherName || "-")}</span></div>
      <time>${formatDate(upload.uploadedAt)}</time>
    </article>
  `).join("") || `<div class="empty-state compact">No marks uploads have been recorded.</div>`;
  els.dashboardActivityFeed.innerHTML = (executive.recentActivity || []).map((activity) => `
    <article>
      <span class="activity-icon">${escapeHtml(activityInitial(activity.action))}</span>
      <div><strong>${escapeHtml(activity.action || "Activity")}</strong><span>${escapeHtml(activity.user || "System")} | ${escapeHtml(activity.newValue || "")}</span></div>
      <time>${formatDate(activity.timestamp)}</time>
    </article>
  `).join("") || `<div class="empty-state compact">No recent activity.</div>`;
...
function activityInitial(action) {
  const words = String(action || "Activity").split(/\s+/).filter(Boolean);
...
function renderStudentProfile() {
  ...
  const reportHistory = (db.reportArchive || []).filter((item) => item.studentId === student.id)
  ...
  els.studentProfileContent.innerHTML = `
    <section class="student-profile-hero">
      <div class="student-profile-photo">
        ${student.photo ? `<img src="${escapeHtml(photoSrc(student.photo))}" alt="${escapeHtml(student.name)}">` : `<span>${escapeHtml(initials || "ST")}</span>`}
      </div>
    ...
    <section class="profile-detail-band">
      <div class="panel-heading"><h3>Report History</h3><span>${reportHistory.length} archived report(s)</span></div>
      <div class="compact-list">
        ${reportHistory.length ? reportHistory.slice(0, 10).map((report) => `<article><strong>${escapeHtml(report.academicYear)} ${escapeHtml(report.term)} ${escapeHtml(report.examType)}</strong><span>${escapeHtml(formatDate(report.issuedAt))} | ${escapeHtml(report.verificationCode)}</span><button type="button" class="secondary" data-open-archive="${escapeHtml(report.id)}">Open</button></article>`).join("") : `<div class="empty-state">No archived report has been generated for this learner yet.</div>`}
      </div>
    </section>
    <section class="profile-detail-band">
  ...
  document.querySelectorAll("[data-open-archive]").forEach((button) => button.addEventListener("click", () => openArchivedReport(button.dataset.openArchive)));
}
...
function renderMarksEntry() {
  ...
  const workflow = currentAssessmentWorkflow();
  const locked = ["Approved", "Locked"].includes(workflow.status);
  renderAssessmentWorkflow(workflow);
  ...
  els.marksBody.innerHTML = learners.length ? learners.map((student) => {
    ...
    return `
      <tr>
        ...
        <td><input class="score-input" type="number" min="0" max="100" data-student-id="${student.id}" data-admission-no="${escapeHtml(student.admissionNo)}" value="${mark?.score ?? ""}" ${locked ? "disabled" : ""}></td>
        <td><textarea class="remark-input" data-student-id="${student.id}" placeholder="Individual subject remark" ${locked ? "disabled" : ""}>${escapeHtml(mark?.remarks || "")}</textarea></td>
  ...
  els.saveMarksBtn.disabled = locked;
  els.uploadMarksCsvBtn.disabled = locked || !pendingMarksImport;
  els.csvInput.disabled = locked;
  decorateMobileTables();
}

function currentAssessmentWorkflow() {
  const context = marksContext();
  const key = [context.academicYear, context.term, context.examType, context.classId, context.subjectId].join("|");
  return db.assessmentWorkflows?.find((item) => item.key === key) || { ...context, key, status: "Draft", history: [] };
}

function renderAssessmentWorkflow(workflow) {
  if (!els.assessmentWorkflowStatus) return;
  const descriptions = {
    Draft: "Teachers can edit marks. Submit when the class and subject are complete.",
    Submitted: "Marks are waiting for Director of Studies review.",
    Approved: "DOS has approved these marks. Editing is disabled while Head Teacher approval is pending.",
    Locked: "Head Teacher approval is complete. Reports can be issued and marks are locked."
  };
  els.assessmentWorkflowStatus.textContent = workflow.status;
  els.assessmentWorkflowStatus.dataset.status = workflow.status.toLowerCase();
  els.assessmentWorkflowDetail.textContent = descriptions[workflow.status] || "Assessment workflow status is unavailable.";
  els.submitAssessmentBtn.hidden = workflow.status !== "Draft" || !hasRole("Super Admin", "School Admin", "DOS", "Class Teacher", "Subject Teacher");
  els.approveAssessmentBtn.hidden = workflow.status !== "Submitted" || !hasRole("Super Admin", "School Admin", "DOS");
  els.lockAssessmentBtn.hidden = workflow.status !== "Approved" || !hasRole("Super Admin", "School Admin", "Head Teacher");
  els.reopenAssessmentBtn.hidden = workflow.status === "Draft" || !hasRole("Super Admin", "School Admin");
}
...
function renderReportArchive() {
  if (!els.reportArchiveList) return;
  const search = reportArchiveSearch.trim().toLowerCase();
  const archive = (db.reportArchive || [])
    .filter((item) => !search || [item.studentName, item.admissionNo, item.className, item.verificationCode, item.term, item.examType]
      .some((value) => String(value || "").toLowerCase().includes(search)))
    .sort((a, b) => new Date(b.issuedAt || 0) - new Date(a.issuedAt || 0))
    .slice(0, 50);
  els.reportArchiveList.innerHTML = archive.length ? archive.map((item) => `
    <article>
      <strong>${escapeHtml(item.studentName || "-")} <small>${escapeHtml(item.admissionNo || "")}</small></strong>
      <span>${escapeHtml(item.className || "-")} | ${escapeHtml(item.academicYear)} ${escapeHtml(item.term)} ${escapeHtml(item.examType)} | ${escapeHtml(formatDate(item.issuedAt))}</span>
      <button type="button" class="secondary" data-open-archive="${escapeHtml(item.id)}">Open</button>
    </article>
  `).join("") : `<div class="empty-state">No archived reports match the current search.</div>`;
  els.reportArchiveList.querySelectorAll("[data-open-archive]").forEach((button) => button.addEventListener("click", () => openArchivedReport(button.dataset.openArchive)));
}

function openArchivedReport(reportId) {
  const report = (db.reportArchive || []).find((item) => item.id === reportId);
  if (!report?.data?.student) return toast("Archived report was not found");
  reportMode = "student";
  els.reportModeLabel.textContent = `Archived report: ${report.studentName || report.data.student.name}`;
  els.reportCards.innerHTML = renderReportPacket(report.data.student);
  navigateTo("reports", "Reports");
}

async function archiveCurrentReports(mode) {
  const classStudents = selectedReportClassStudents();
  if (!classStudents.length) throw new Error("Select a class with learners first");
  const body = mode === "class"
    ? { classId: selectedReportClassId }
    : { studentIds: [selectedReportStudentId || classStudents[0].id] };
  const response = await api("/api/reports/archive", { method: "POST", body: JSON.stringify(body) });
  await loadData();
  toast(`${response.archived || 0} report archived`);
}
...
async function runVerification(code, container) {
  ...
  try {
    ...
    container.innerHTML = `
      <section class="verified-banner"><strong>Verified by Shule Results Management System</strong><span>Report issued ${formatDate(student.reportIssueDate || verified.verifiedAt)} | Checked ${formatDate(verified.verifiedAt)}</span></section>
      <section class="verified-student-card">
        <div class="student-profile-photo">${student.photo ? `<img src="${escapeHtml(photoSrc(student.photo))}" alt="${escapeHtml(student.name)}">` : `<span>${escapeHtml(student.name.split(/\s+/).map((part) => part[0]).slice(0, 2).join(""))}</span>`}</div>
...
async function loadUsers() {
  if (!hasRole("Super Admin", "School Admin")) return;
...
function renderUsers() {
  const roles = ["Super Admin", "School Admin", "Head Teacher", "DOS", "Class Teacher", "Subject Teacher", "Bursar", "Parent", "Viewer"]
    .filter((role) => currentUser?.role === "Super Admin" || role !== "Super Admin");
  ...
  els.userList.innerHTML = staffUsers.map((user) => `
    <article>
      ...
      <div class="user-role-control">
        <select data-user-role="${escapeHtml(user.id)}">${roles.map((role) => `<option ${role === user.role ? "selected" : ""}>${role}</option>`).join("")}</select>
...
async function changeAssessmentWorkflow(workflowAction) {
  const labels = {
    submit: "Submit this assessment to the Director of Studies?",
    approve: "Approve these marks as Director of Studies?",
    lock: "Lock these approved marks for official report generation?",
    reopen: "Reopen this assessment and allow marks editing again?"
  };
  if (!window.confirm(labels[workflowAction] || "Continue with this workflow action?")) return;
  const response = await api("/api/marks", {
    method: "POST",
    body: JSON.stringify({ action: "workflow", workflowAction, ...marksContext() })
  });
  toast(`Assessment moved to ${response.workflow.status}`);
  await loadData();
}
...
function renderPromotionRuleForm() {
  if (!els.promotionRuleForm || !db?.promotionRules) return;
  const classId = els.promotionRuleClassSelect.value || db.classes[0]?.id;
  const rule = {
    ...db.promotionRules,
    ...(db.promotionRules.perClass?.[classId] || {})
  };
  els.promotionRuleForm.elements.minAverage.value = rule.minAverage ?? 50;
  els.promotionRuleForm.elements.maxFailedSubjects.value = rule.maxFailedSubjects ?? 2;
  els.promotionRuleForm.elements.requireCompleteMarks.checked = rule.requireCompleteMarks !== false;
  const mandatory = new Set(rule.mandatorySubjectIds || []);
  [...els.promotionMandatorySubjectsSelect.options].forEach((option) => {
    option.selected = mandatory.has(option.value);
  });
}

async function savePromotionRule(event) {
  event.preventDefault();
  const classId = els.promotionRuleClassSelect.value;
  const body = {
    promotionRules: {
      perClass: {
        [classId]: {
          minAverage: Number(event.currentTarget.elements.minAverage.value),
          maxFailedSubjects: Number(event.currentTarget.elements.maxFailedSubjects.value),
          requireCompleteMarks: event.currentTarget.elements.requireCompleteMarks.checked,
          mandatorySubjectIds: [...els.promotionMandatorySubjectsSelect.selectedOptions].map((option) => option.value)
        }
      }
    }
  };
  await api("/api/settings", { method: "POST", body: JSON.stringify(body) });
  toast(`Promotion rule saved for ${classById(classId)?.name || classId}`);
  await loadData();
}

async function rollbackLatestPromotion() {
  const latest = [...(db.promotionHistory || [])].reverse().find((item) => !item.rolledBackAt);
  if (!latest) return toast("No approved promotion batch is available to roll back");
  const reason = window.prompt("Reason for rolling back this promotion approval:");
  if (reason === null) return;
  if (!window.confirm(`Restore ${latest.rows?.length || 0} learner record(s) to their previous classes?`)) return;
  await api("/api/promotions", {
    method: "POST",
    body: JSON.stringify({ action: "rollback", historyId: latest.id, reason: reason.trim() || "Administrative correction" })
  });
  toast("Promotion approval rolled back");
  await loadData();
}
...
els.submitAssessmentBtn?.addEventListener("click", () => changeAssessmentWorkflow("submit").catch((error) => toast(error.message)));
els.approveAssessmentBtn?.addEventListener("click", () => changeAssessmentWorkflow("approve").catch((error) => toast(error.message)));
els.lockAssessmentBtn?.addEventListener("click", () => changeAssessmentWorkflow("lock").catch((error) => toast(error.message)));
els.reopenAssessmentBtn?.addEventListener("click", () => changeAssessmentWorkflow("reopen").catch((error) => toast(error.message)));
...
els.rollbackPromotionBtn?.addEventListener("click", () => rollbackLatestPromotion().catch((error) => toast(error.message)));
els.promotionRuleForm?.addEventListener("submit", (event) => savePromotionRule(event).catch((error) => toast(error.message)));
els.promotionRuleClassSelect?.addEventListener("change", renderPromotionRuleForm);
...
els.reportArchiveSearchInput?.addEventListener("input", () => {
  reportArchiveSearch = els.reportArchiveSearchInput.value;
  renderReportArchive();
});
